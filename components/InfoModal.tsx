  ...
  return (
    <>
      <button
        ...
        aria-label={t.modal.buttonTitle}
        title={t.modal.buttonTitle}
      >
        ...
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
          ...
          <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-xl shadow-2xl p-6 md:p-8 transition-all
            ...
            <div className="space-y-6 text-sm leading-relaxed text-gray-600 dark:text-gray-300">
              <section>
                <h3 className="font-bold text-lg mb-2 text-gray-900 dark:text-white">{t.modal.byokTitle}</h3>
                <p>
                  {t.modal.byokText1}
                </p>
                <ul className="list-disc pl-5 mt-2 space-y-1">
                  {t.modal.byokList.map((item, i) => (
                    <li key={i}>{item}</li>
                  ))}
                </ul>
              </section>
              ...
              <section>
                <h3 className="font-bold text-lg mb-2 text-gray-900 dark:text-white">{t.modal.importTitle}</h3>
                <ol className="list-decimal pl-5 mt-2 space-y-2">
                  {t.modal.importList.map((item, i) => (
                    <li key={i}>{item}</li>
                  ))}
                </ol>
              </section>
              ...
              <section>
                <h3 className="font-bold text-lg mb-2 text-gray-900 dark:text-white">{t.modal.openSourceTitle}</h3>
                <p>
                  {t.modal.openSourceText}
                </p>
                <div className="mt-3">
                  <a 
